...
class hybrid_DReductionC():
    ...
    def _fisher_feature_ranker(self, X, y):
        ...
        worstN- is thhe percentagge we want to drop
        output:
    ...
    def _multi_stra_fselection(self, X, y):
        """
        input:
        the dataframe along with the target class annd the percentage of worst to drop
        output:
        name of features after droping the worst features
        """
    ...
    def _MICI(self, f_feature, s_feature):
        ...
        returns:
        the function returns the MICI similiratiy measure value for the two features.
        """
        try:
    ...
    def _Heir_clustering(self, X):
        """
        describtion: hidden class method that perform the heirarachial clustering of thhe features based on
        MICI measurments as a distance parameter.
        X param:the Features to be clusterd in AgglomerativeClustering
        return:
        returns the clustering labels in dict, the labels would be the keys and for each cluster label
        the items would be the fetaures names in that cluster.
        """
    ...
    def _PCA_FS(self, X):
        """
        calcultes the PCA first compenetent for each cluster and returns the "scoring" for each
        ...
        according to the var of first compenet in the PCA
        """
        if self.scaled:
    ...
    def _hybrid_DReduction(self, X, y):
        ...
        dimension reduction alogrithm in the reference.
    ...
    def fit(self, X, y):
        """
        describtion: performing the fit, aka calling the wrapper function _hybrid_DReduction
        X param: the features in a dataframe type
        y param: the labels for each sample as passed in the X
        return:
        self
