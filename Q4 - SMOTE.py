...
for dataset in os.listdir(main_path):
    if dataset.endswith("csv"):
        ...
        if dataset_name == "Carcinom_result":
            smoote = BorderlineSMOTE(sampling_strategy="not majority",random_state=42,m_neighbors=4)
        elif dataset_name =="ALL_result"  :
            print("elif")
            # smoote = BorderlineSMOTE(sampling_strategy="not majority", m_neighbors=2, random_state=42)
            smoote = BorderlineSMOTE(sampling_strategy="not majority", m_neighbors=1, k_neighbors=2, random_state=42)
        elif dataset_name == "nakayama_result":
            smoote = BorderlineSMOTE(sampling_strategy="not majority", m_neighbors=1, k_neighbors=2,random_state=42)
        elif dataset_name == "bcellViper_result" :
            smoote = BorderlineSMOTE(sampling_strategy="not majority", m_neighbors=2,k_neighbors=2, random_state=42)
            # smoote = BorderlineSMOTE()
        else:
            print("else")
            smoote = BorderlineSMOTE(sampling_strategy="not majority",random_state=42)
        ...
        results = pd.concat(
            [pd.DataFrame.from_dict([{"param_reduce_dim__score_func": str(score_func_AUG) + "__aug"}]), results], axis=1)
        score_func_AUG.__str__()
        results = pd.concat(
            [pd.DataFrame.from_dict([{"k": str(k_AUG) }]), results], axis=1)
